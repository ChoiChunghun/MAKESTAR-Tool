...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 53.41 7.55">
  ...
  const TEXT_STYLE_DEFAULTS = {
    textA: {
      ...
      verticalShift: 0,
      lineBreakScaleFloor: 1
    },
    textB: {
      fontScale: 1,
      trackingEm: TEXT_TRACKING_EM,
      verticalShift: 0,
      // Keep clearly one-line event defaults in the 1-line layout,
      // and only switch to 2-line layouts once more substantial shrink is needed.
      lineBreakScaleFloor: 0.95,
      fitSample: "EVENTFORMAT IN CITYNAME",
      transformText: (value) => String(value ?? "").toUpperCase()
    }
  };
  ...
  function resolveStyledText(text, style){
    const normalized = normalizeMultiline(text);
    return style?.transformText ? style.transformText(normalized) : normalized;
  }
  ...
  function inferLineCount(ctx, text, targetHeight, weight, maxWidth, style){
    ...
    const styledText = resolveStyledText(text, resolvedStyle);
    const explicitLines = splitExplicitLines(styledText, 2).length;
    if(explicitLines > 1){
      return Math.max(1, Math.min(2, explicitLines));
    }
    const sample = resolvedStyle.fitSample || styledText.replace(/\n+/g, " ").trim() || "Hg";
    ...
    const floorRatio = resolvedStyle.lineBreakScaleFloor ?? 1;
    const floorSize = referenceSize * floorRatio;

    for(let size = referenceSize; size >= floorSize; size -= 0.5){
      setFont(ctx, size, weight);
      const trackingPx = size * resolvedStyle.trackingEm;
      const singleLine = wrapTextWithNewlines(ctx, styledText, maxWidth, trackingPx, 1);
      const widthsOk = singleLine.every((line) => measureTracking(ctx, line, trackingPx) <= maxWidth + 0.5);
      if(singleLine.length === 1 && widthsOk){
        return 1;
      }
    }

    setFont(ctx, referenceSize, weight);
    const trackingPx = referenceSize * resolvedStyle.trackingEm;
    return Math.max(1, Math.min(2, wrapTextWithNewlines(ctx, styledText, maxWidth, trackingPx, 2).length));
  }
  ...
  function drawTextBlock(ctx, text, boxes, weight, maxWidth, minPx, probeHeight, style){
    ...
    let lines = wrapTextWithNewlines(ctx, styledText, maxWidth, trackingPx, boxes.length);
    const floor = minPx ?? 6;
    let fallback = null;
    while(size >= floor){
      setFont(ctx, size, weight);
      trackingPx = size * resolvedStyle.trackingEm;
      lines = wrapTextWithNewlines(ctx, styledText, maxWidth, trackingPx, boxes.length);
      const widthsOk = lines.every((line) => measureTracking(ctx, line, trackingPx) <= maxWidth + 0.5);
      const expectedLineCount = styledText.trim().length === 0 ? 1 : boxes.length;
    ...
    if(
      fallback &&
      (lines.length !== (styledText.trim().length === 0 ? 1 : boxes.length) ||
  ...
  function boot(){
    ...
    const isBadgeOnlyMode = searchParams?.get("mode") === "badge-only";
    badgeCanvas.style.boxShadow = isBadgeOnlyMode ? "none" : "0 0 0 2px #FFBF00";
    badgeCanvas.style.margin = isBadgeOnlyMode ? "0 auto" : "2px auto";
    badgeCanvas.style.borderRadius = isBadgeOnlyMode ? "10px" : "1px";
    ...
    function drawDate(ctx, text, box){
      const targetBox = box || dateBox;
      const fitted = fitTextToBox(ctx, text || "YYYY.MM.DD", targetBox, 700, TEXT_TRACKING_EM, 6);
      ctx.save();
      ctx.fillStyle = "#FFFFFF";
      setFont(ctx, fitted.size, 700);
      ctx.textBaseline = "alphabetic";
      const baseline = baselineForBBoxCenter(ctx, text, targetBox.y + targetBox.h / 2);
      drawTextTracking(ctx, text, targetBox.x + targetBox.w / 2, baseline, "center", fitted.trackingPx);
      ctx.restore();
    }
    ...
    function drawBadge(ctx, data, opts){
      ...
      const stateDateBox = state.date
        ? {
            x: state.date.x ?? dateBox.x,
            y: state.date.y,
            w: state.date.w ?? dateBox.w,
            h: state.date.h
          }
        : dateBox;
      ...
      drawDate(ctx, data.textC, stateDateBox);
